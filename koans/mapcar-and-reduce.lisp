...
(define-test map
  ...
  (let ((string "lorem ipsum"))
    ...
    (assert-equal '(#\L #\O #\R #\E #\M #\space #\I #\P #\S #\U #\M) (map 'list #'char-upcase string))
    ;; Not all vectors containing characters are strings.
    (assert-equalp "LOREM IPSUM" (map '(vector t) #'char-upcase string))))
...
(define-test reduce
  ...
  (assert-equal (expt (expt (expt (expt 1 2) 3) 4) 5) (reduce #'expt '(1 2 3 4 5))))
...
(define-test reduce-from-end
  ...
  (let ((numbers '(2 3 2)))
    ...
    (assert-equal (expt 2 (expt 3 2)) (reduce #'expt numbers :from-end t))))
